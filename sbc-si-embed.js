...
function loadShopify() {
  ...
  fetch(host, options).then((response) => { // eslint-disable-line
    ...
    if (info.fullFeed) {
      ...
      const size = Number(sbc_global_styles.storefrontCells);
      const arrayOfArrays = [];
      ...
      let limiter = arrayOfArrays.length;
      if (limiter > 6) {
        limiter = 6;
      }
      for (let iter = 0; iter < limiter; iter++) {
        sbc_global_current_row = iter;
        const currentArray = arrayOfArrays[iter];
        // alert(currentArray.length);
        ...
        for (let i = 0; i < currentArray.length; i++) {
          if (info.itemsObj && info.itemsObj[currentArray[i].id] && info.itemsObj[currentArray[i].id].items && info.itemsObj[currentArray[i].id].items.length) {
            ...
            // console.log(info.itemsObj[currentArray[i].id]);
      ...
      if (Object.keys(info.itemsObj).length > (4 * (limiter))) {
...
function addAnotherRow() {
  ...
  if (sbc_global_full_image_array_array[sbc_global_current_row]) {
    ...
    bigDiv.appendChild(rowDiv);
    // do a calculation
    let numberInNextRow = 0;
    let newCurrentArray = sbc_global_full_image_array_array[sbc_global_current_row + 1];
    if (!newCurrentArray) {
      newCurrentArray = [];
    }
    for (let i = 0; i < newCurrentArray.length; i++) {
      if (info.itemsObj && info.itemsObj[newCurrentArray[i].id] && info.itemsObj[newCurrentArray[i].id].items && info.itemsObj[newCurrentArray[i].id].items.length) {
        numberInNextRow += 1;
      }
    }
...
function openConfirmModal(item, shopifyStoreURL) {
  ...
  continueButton.style.cssText = 'text-align:center;font-size:24px;height:35px;width:50%;cursor:pointer;border:none;color:white;background-color:black';
